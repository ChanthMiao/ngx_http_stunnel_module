...
typedef void (*ngx_http_stunnel_upstream_handler_pt)(ngx_http_request_t *r,
...
struct ngx_http_stunnel_upstream_s {
    ...
    ngx_http_stunnel_upstream_handler_pt read_event_handler;
    ngx_http_stunnel_upstream_handler_pt write_event_handler;
...
struct ngx_http_stunnel_address_s {
    ngx_addr_t *addr;
...
static ngx_command_t ngx_http_stunnel_commands[] = {
    ...
    {ngx_string("stunnel"), NGX_HTTP_LOC_CONF | NGX_CONF_NOARGS, ngx_http_stunnel,
     NGX_HTTP_LOC_CONF_OFFSET, offsetof(ngx_http_stunnel_loc_conf_t, accept_connect), NULL},

    {ngx_string("stunnel_allow"),
     NGX_HTTP_MAIN_CONF | NGX_HTTP_SRV_CONF | NGX_HTTP_LOC_CONF | NGX_CONF_1MORE,
     ngx_http_stunnel_allow, NGX_HTTP_LOC_CONF_OFFSET, 0, NULL},

    {ngx_string("stunnel_read_timeout"),
     NGX_HTTP_MAIN_CONF | NGX_HTTP_SRV_CONF | NGX_HTTP_LOC_CONF | NGX_CONF_TAKE1,
     ngx_conf_set_msec_slot, NGX_HTTP_LOC_CONF_OFFSET,
     offsetof(ngx_http_stunnel_loc_conf_t, read_timeout), NULL},

    {ngx_string("stunnel_send_timeout"),
     NGX_HTTP_MAIN_CONF | NGX_HTTP_SRV_CONF | NGX_HTTP_LOC_CONF | NGX_CONF_TAKE1,
     ngx_conf_set_msec_slot, NGX_HTTP_LOC_CONF_OFFSET,
     offsetof(ngx_http_stunnel_loc_conf_t, send_timeout), NULL},

    {ngx_string("stunnel_connect_timeout"),
     NGX_HTTP_MAIN_CONF | NGX_HTTP_SRV_CONF | NGX_HTTP_LOC_CONF | NGX_CONF_TAKE1,
     ngx_conf_set_msec_slot, NGX_HTTP_LOC_CONF_OFFSET,
    ...
    {ngx_string("stunnel_send_lowat"),
     ...
     offsetof(ngx_http_stunnel_loc_conf_t, send_lowat), NULL},

    {ngx_string("stunnel_address"),
     NGX_HTTP_MAIN_CONF | NGX_HTTP_SRV_CONF | NGX_HTTP_LOC_CONF | NGX_CONF_TAKE1,
     ngx_http_set_complex_value_slot, NGX_HTTP_LOC_CONF_OFFSET,
     offsetof(ngx_http_stunnel_loc_conf_t, address), NULL},

    {ngx_string("stunnel_bind"),
...
static ngx_http_module_t ngx_http_stunnel_module_ctx = {
    ngx_http_stunnel_add_variables, /* preconfiguration */
    ngx_http_stunnel_init,          /* postconfiguration */
...
ngx_module_t ngx_http_stunnel_module = {NGX_MODULE_V1,
                                        &ngx_http_stunnel_module_ctx, /* module context */
                                        ngx_http_stunnel_commands,    /* module directives */
...
static ngx_http_variable_t ngx_http_stunnel_vars[] = {

    {ngx_string("connect_addr"), NULL, ngx_http_stunnel_connect_addr_variable, 0,
     NGX_HTTP_VAR_NOCACHEABLE, 0},

    {ngx_string("stunnel_connect_timeout"), ngx_http_stunnel_variable_set_time,
     ngx_http_stunnel_variable_get_time, offsetof(ngx_http_stunnel_ctx_t, connect_timeout),
     NGX_HTTP_VAR_NOCACHEABLE | NGX_HTTP_VAR_CHANGEABLE, 0},

    {ngx_string("stunnel_read_timeout"), ngx_http_stunnel_variable_set_time,
     ngx_http_stunnel_variable_get_time, offsetof(ngx_http_stunnel_ctx_t, read_timeout),
     NGX_HTTP_VAR_NOCACHEABLE | NGX_HTTP_VAR_CHANGEABLE, 0},

    {ngx_string("stunnel_send_timeout"), ngx_http_stunnel_variable_set_time,
     ngx_http_stunnel_variable_get_time, offsetof(ngx_http_stunnel_ctx_t, send_timeout),
     NGX_HTTP_VAR_NOCACHEABLE | NGX_HTTP_VAR_CHANGEABLE, 0},
 ...
 */
static void __ngx_inet_set_port(struct sockaddr *sa, in_port_t port) {
...
static ngx_int_t __ngx_parse_addr_port(ngx_pool_t *pool, ngx_addr_t *addr, u_char *text,
                                       size_t len) {
    u_char *p, *last;
    size_t plen;
    ngx_int_t rc, port;
...
static ngx_int_t ngx_http_stunnel_get_peer(ngx_peer_connection_t *pc, void *data) {
    return NGX_OK;
}

static ngx_int_t ngx_http_stunnel_test_connect(ngx_connection_t *c) {
    int err;
    socklen_t len;
...
#endif
    {
        ...
        if (err) {
            c->log->action = "connecting to upstream";
            (void)ngx_connection_error(c, err, "stunnel: upstream connect failed");
            return NGX_ERROR;
        }
    }
...
static void ngx_http_stunnel_finalize_request(ngx_http_request_t *r,
    ...
    ngx_log_debug1(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,
                   "stunnel: finalize upstream request: %i", rc);
    ...
    if (u->peer.connection) {
        ngx_log_debug1(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,
                       "stunnel: close upstream connection: %d", u->peer.connection->fd);
    ...
    if (rc == NGX_HTTP_REQUEST_TIME_OUT || rc == NGX_HTTP_CLIENT_CLOSED_REQUEST) {
...
static void ngx_http_stunnel_send_connection_established(ngx_http_request_t *r) {
    ngx_int_t n;
    ngx_buf_t *b;
    ngx_connection_t *c;
    ...
    ngx_log_debug0(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,
                   "stunnel: send 200 connection established");
        ...
        if (n >= 0) {
            ...
            if (b->pos == b->last) {
                ngx_log_debug0(NGX_LOG_DEBUG_HTTP, c->log, 0,
                               "stunnel: sent 200 connection established");
                ...
                r->write_event_handler = ngx_http_stunnel_write_downstream;
                r->read_event_handler = ngx_http_stunnel_read_downstream;

                if (ngx_handle_write_event(c->write, clcf->send_lowat) != NGX_OK) {
                    ngx_http_stunnel_finalize_request(r, u, NGX_HTTP_INTERNAL_SERVER_ERROR);
                    return;
                }
    ...
    if (n == NGX_ERROR) {
        ngx_http_stunnel_finalize_request(r, u, NGX_ERROR);
        return;
    }

    /* n == NGX_AGAIN */
    r->write_event_handler = ngx_http_stunnel_send_handler;
...
static void ngx_http_stunnel_tunnel(ngx_http_request_t *r, ngx_uint_t from_upstream,
                                    ngx_uint_t do_write) {
    size_t size;
    ssize_t n;
    ngx_buf_t *b;
    ...
    ngx_log_debug2(NGX_LOG_DEBUG_HTTP, c->log, 0, "stunnel: tunnel fu:%ui write:%ui",
    ...
    if ((upstream->read->eof && u->buffer.pos == u->buffer.last) ||
        ...
        ngx_log_debug0(NGX_LOG_DEBUG_HTTP, c->log, 0, "stunnel: tunnel done");
        ngx_http_stunnel_finalize_request(r, u, 0);
        return;
    }
...
static void ngx_http_stunnel_read_downstream(ngx_http_request_t *r) {
    ...
    if (r->connection->read->timedout) {
        ...
        ngx_log_error(NGX_LOG_ERR, r->connection->log, 0, "stunnel: client read timed out");
        ngx_http_stunnel_finalize_request(r, ctx->u, NGX_HTTP_REQUEST_TIME_OUT);
        return;
    }

    ngx_http_stunnel_tunnel(r, 0, 0);
}
...
static void ngx_http_stunnel_write_downstream(ngx_http_request_t *r) {
    ...
    if (r->connection->write->timedout) {
        r->connection->timedout = 1;
        ngx_log_error(NGX_LOG_ERR, r->connection->log, 0, "stunnel: client write timed out");
        ngx_http_stunnel_finalize_request(r, ctx->u, NGX_HTTP_REQUEST_TIME_OUT);
        return;
    }

    ngx_http_stunnel_tunnel(r, 1, 1);
}

static void ngx_http_stunnel_read_upstream(ngx_http_request_t *r,
                                           ngx_http_stunnel_upstream_t *u) {
    ngx_connection_t *c;
    ngx_http_stunnel_ctx_t *ctx;

    ngx_log_debug0(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "stunnel: upstream read handler");
    ...
    if (c->read->timedout) {
        ngx_log_error(NGX_LOG_ERR, c->log, 0, "stunnel: upstream read timed out (peer:%V)",
                      u->peer.name);
        ngx_http_stunnel_finalize_request(r, u, NGX_HTTP_GATEWAY_TIME_OUT);
        return;
    }

    if (!ctx->send_established && ngx_http_stunnel_test_connect(c) != NGX_OK) {
        ngx_http_stunnel_finalize_request(r, u, NGX_HTTP_BAD_GATEWAY);
        return;
    }
    ...
    ngx_http_stunnel_tunnel(r, 1, 0);
}

static void ngx_http_stunnel_write_upstream(ngx_http_request_t *r,
                                            ngx_http_stunnel_upstream_t *u) {
    ngx_connection_t *c;
    ngx_http_stunnel_ctx_t *ctx;
    ...
    ngx_log_debug0(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,
                   "stunnel: upstream write handler");

    if (c->write->timedout) {
        ngx_log_error(NGX_LOG_ERR, c->log, 0, "stunnel: upstream %s timed out (peer:%V)",
    ...
    if (!ctx->send_established) {
        ngx_http_stunnel_send_connection_established(r);
        return;
    }
...
static void ngx_http_stunnel_send_handler(ngx_http_request_t *r) {
    ...
    ngx_log_debug0(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,
                   "stunnel: send connection established handler");

    if (c->write->timedout) {
        c->timedout = 1;
        ngx_log_error(NGX_LOG_ERR, c->log, 0, "stunnel: client write timed out");
        ngx_http_stunnel_finalize_request(r, ctx->u, NGX_HTTP_REQUEST_TIME_OUT);
        return;
    }
...
static void ngx_http_stunnel_upstream_handler(ngx_event_t *ev) {
    ngx_connection_t *c;
    ...
    ngx_log_debug2(NGX_LOG_DEBUG_HTTP, c->log, 0, "stunnel: upstream handler: \"%V:%V\"",
...
static void ngx_http_stunnel_process_connect(ngx_http_request_t *r,
                                             ngx_http_stunnel_upstream_t *u) {
    ngx_int_t rc;
    ngx_connection_t *c;
    ...
    if (ngx_http_stunnel_set_local(r, u, u->conf->local) != NGX_OK) {
        ngx_http_stunnel_finalize_request(r, u, NGX_HTTP_INTERNAL_SERVER_ERROR);
        return;
    }
    ...
    pc->get = ngx_http_stunnel_get_peer;

    rc = ngx_event_connect_peer(&u->peer);

    ngx_log_debug1(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,
                   "stunnel: ngx_event_connect_peer() returns %i", rc);
    ...
    if (rc == NGX_BUSY) {
        ngx_log_error(NGX_LOG_ERR, r->connection->log, 0, "stunnel: no live connection");
        ngx_http_stunnel_finalize_request(r, u, NGX_HTTP_BAD_GATEWAY);
        return;
    }

    if (rc == NGX_DECLINED) {
        ngx_log_error(NGX_LOG_ERR, r->connection->log, 0, "stunnel: connection error");
        ngx_http_stunnel_finalize_request(r, u, NGX_HTTP_BAD_GATEWAY);
        return;
    }
    ...
    c->write->handler = ngx_http_stunnel_upstream_handler;
    c->read->handler = ngx_http_stunnel_upstream_handler;

    u->write_event_handler = ngx_http_stunnel_write_upstream;
    u->read_event_handler = ngx_http_stunnel_read_upstream;
...
static void ngx_http_stunnel_resolve_handler(ngx_resolver_ctx_t *ctx) {
    ngx_connection_t *c;
    ngx_http_request_t *r;
    ngx_http_upstream_resolved_t *ur;
    ngx_http_stunnel_upstream_t *u;
    ...
    ngx_log_debug0(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "stunnel: resolve handler");

    if (ctx->state) {
        ngx_log_error(NGX_LOG_ERR, r->connection->log, 0,
                      "stunnel: %V could not be resolved (%i: %s)", &ctx->name, ctx->state,
...
#if (NGX_DEBUG)
    {
        ...
        for (i = 0; i < ctx->naddrs; i++) {
            addr.len = ngx_sock_ntop(ur->addrs[i].sockaddr, ur->addrs[i].socklen, text,
                                     NGX_SOCKADDR_STRLEN, 0);

            ngx_log_debug1(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,
                           "stunnel: name was resolved to %V", &addr);
        }
#else
        ngx_uint_t i;
        in_addr_t addr;
        ...
        for (i = 0; i < ctx->naddrs; i++) {
            ...
            ngx_log_debug4(NGX_LOG_DEBUG_HTTP, c->log, 0,
                           "stunnel: name was resolved to %ud.%ud.%ud.%ud", (addr >> 24) & 0xff,
    ...
    if (ngx_http_stunnel_create_peer(r, ur) != NGX_OK) {
        ngx_http_stunnel_finalize_request(r, u, NGX_HTTP_INTERNAL_SERVER_ERROR);
        goto failed;
    }
    ...
    ngx_http_stunnel_process_connect(r, u);
...
static ngx_int_t ngx_http_stunnel_hmac(ngx_http_request_t *r) {
    ...
    ngx_http_stunnel_loc_conf_t *slcf;
    u_char hash[16];
    u_char buffer[ngx_base64_encoded_length(16)];
...
static ngx_int_t ngx_http_stunnel_upstream_create(ngx_http_request_t *r,
    ...
    u = ngx_pcalloc(r->pool, sizeof(ngx_http_stunnel_upstream_t));
    if (u == NULL) {
        return NGX_ERROR;
    }
...
static void ngx_http_stunnel_check_broken_connection(ngx_http_request_t *r, ngx_event_t *ev) {
    int n;
    char buf[1];
    ngx_err_t err;
    ngx_int_t event;
    ...
    ngx_log_debug3(NGX_LOG_DEBUG_HTTP, ev->log, 0,
                   "stunnel: check client, write event:%d, \"%V:%V\"", ev->write,
                   &r->connect_host, &r->connect_port);
    ...
    if (c->error) {
        ...
        ngx_http_stunnel_finalize_request(r, u, NGX_HTTP_CLIENT_CLOSED_REQUEST);
    ...
    if (ngx_event_flags & NGX_USE_KQUEUE_EVENT) {
        ...
        if (u->peer.connection) {
            ngx_log_error(NGX_LOG_INFO, ev->log, ev->kq_errno,
                          "stunnel: kevent() reported that client "
    ...
    ngx_log_debug1(NGX_LOG_DEBUG_HTTP, ev->log, err, "stunnel: upstream recv(): %d", n);
    ...
    if (u->peer.connection) {
        ngx_log_error(NGX_LOG_INFO, ev->log, err,
                      "stunnel: client prematurely closed connection, "
                      "so upstream connection is closed too");
        ngx_http_stunnel_finalize_request(r, u, NGX_HTTP_CLIENT_CLOSED_REQUEST);
        return;
    }

    ngx_log_error(NGX_LOG_INFO, ev->log, err, "stunnel: client prematurely closed connection");
...
static void ngx_http_stunnel_rd_check_broken_connection(ngx_http_request_t *r) {
    ngx_http_stunnel_check_broken_connection(r, r->connection->read);
}

static void ngx_http_stunnel_wr_check_broken_connection(ngx_http_request_t *r) {
    ngx_http_stunnel_check_broken_connection(r, r->connection->write);
}
...
static ngx_int_t ngx_http_stunnel_handler(ngx_http_request_t *r) {
    ...
    ctx->read_timeout = slcf->read_timeout;

    if (ngx_http_stunnel_upstream_create(r, ctx) != NGX_OK) {
    ...
    if (slcf->address) {
        if (ngx_http_complex_value(r, slcf->address, &url.url) != NGX_OK) {
            return NGX_HTTP_INTERNAL_SERVER_ERROR;
        }
    ...
    ngx_log_debug1(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,
                   "stunnel: connect handler: parse url: %V", &url.url);

    if (ngx_parse_url(r->pool, &url) != NGX_OK) {
        if (url.err) {
            ngx_log_error(NGX_LOG_ERR, r->connection->log, 0,
                          "stunnel: %s in connect host \"%V\"", url.err, &url.url);
    ...
    r->read_event_handler = ngx_http_stunnel_rd_check_broken_connection;
    r->write_event_handler = ngx_http_stunnel_wr_check_broken_connection;
    ...
    if (url.addrs) {
        ngx_log_debug0(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,
                       "stunnel: upstream address given directly");
    ...
    if (u->resolved->sockaddr) {
        rc = ngx_http_stunnel_sock_ntop(r, u);
    ...
    if (rctx == NULL) {
        ngx_log_error(NGX_LOG_ERR, r->connection->log, 0,
                      "stunnel: failed to start the resolver");
        return NGX_HTTP_INTERNAL_SERVER_ERROR;
    }

    if (rctx == NGX_NO_RESOLVER) {
        ngx_log_error(NGX_LOG_ERR, r->connection->log, 0,
                      "stunnel: no resolver defined to resolve %V", &r->connect_host);
...
#endif
    rctx->handler = ngx_http_stunnel_resolve_handler;
    rctx->data = u;
    ...
    if (ngx_resolve_name(rctx) != NGX_OK) {
        ngx_log_debug0(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,
                       "stunnel: fail to run resolver immediately");
...
static ngx_int_t ngx_http_stunnel_allow_handler(ngx_http_request_t *r,
    ...
    if (slcf->allow_port_all) {
        allow = 1;

    } else if (slcf->allow_ports) {
        ports = slcf->allow_ports->elts;

        for (i = 0; i < slcf->allow_ports->nelts; i++) {
            /*
             * connect_port == port
             * OR
...
static char *ngx_http_stunnel_allow(ngx_conf_t *cf, ngx_command_t *cmd, void *conf) {
    ...
    ngx_uint_t i;
    ngx_str_t *value;
    ngx_http_stunnel_loc_conf_t *slcf = conf;

    if (slcf->allow_ports != NGX_CONF_UNSET_PTR) {
        return "is duplicate";
    }

    slcf->allow_ports = ngx_array_create(cf->pool, 2, sizeof(in_port_t[2]));
    if (slcf->allow_ports == NULL) {
    ...
    for (i = 1; i < cf->args->nelts; i++) {
        ...
        if (p != NULL) {
            ...
            if (port == NGX_ERROR || port < 1 || port > 65535 || eport == NGX_ERROR ||
                eport < 1 || eport > 65535 || port > eport) {
                ngx_conf_log_error(NGX_LOG_EMERG, cf, 0,
                                   "invalid port range \"%V\" in \"%V\" directive", &value[i],
                                   &cmd->name);
                return NGX_CONF_ERROR;
            }
            ...
            if (port == NGX_ERROR || port < 1 || port > 65535) {
                ngx_conf_log_error(NGX_LOG_EMERG, cf, 0,
                                   "invalid value \"%V\" in \"%V\" directive", &value[i],
                                   &cmd->name);
                return NGX_CONF_ERROR;
            }
        ...
        ports = ngx_array_push(slcf->allow_ports);
        if (ports == NULL) {
            return NGX_CONF_ERROR;
        }
...
char *ngx_http_stunnel_bind(ngx_conf_t *cf, ngx_command_t *cmd, void *conf) {
    char *p = conf;

    ngx_int_t rc;
    ngx_str_t *value;
    ngx_http_complex_value_t cv;
    ngx_http_stunnel_address_t **plocal, *local;
    ngx_http_compile_complex_value_t ccv;

    plocal = (ngx_http_stunnel_address_t **)(p + cmd->offset);
    ...
    local = ngx_pcalloc(cf->pool, sizeof(ngx_http_stunnel_address_t));
    if (local == NULL) {
        return NGX_CONF_ERROR;
    }
    ...
    *plocal = local;
    ...
    if (cv.lengths) {
        ...
        *local->value = cv;
        ...
        rc = __ngx_parse_addr_port(cf->pool, local->addr, value[1].data, value[1].len);
...
static ngx_int_t ngx_http_stunnel_set_local(ngx_http_request_t *r,
    ...
    if (rc != NGX_OK) {
        ngx_log_error(NGX_LOG_ERR, r->connection->log, 0,
                      "stunnel: invalid local address \"%V\"", &val);
        return NGX_OK;
    }
...
static char *ngx_http_stunnel_key(ngx_conf_t *cf, ngx_command_t *cmd, void *conf) {
    char *p = conf;
    ngx_str_t key, *value;
    ngx_http_stunnel_hmac_t *field;
    ngx_uint_t i;

    field = (ngx_http_stunnel_hmac_t *)(p + cmd->offset);
    ...
    value = cf->args->elts;
    key = value[1];
    /* The key should be in the format of uuid (v4), with fixed length 36.*/
    if (ngx_http_stunnel_uuid_check_v4(&key) != NGX_OK) {
    ...
    for (i = 0; i < 64; i++) {
        ...
        field->opad.data[i] ^= 0x5c;
    }
    /* Hamc-md5 key Setup successfully. */
    return NGX_CONF_OK;
}
...
static void *ngx_http_stunnel_create_loc_conf(ngx_conf_t *cf) {
    ...
    conf = ngx_pcalloc(cf->pool, sizeof(ngx_http_stunnel_loc_conf_t));
    if (conf == NULL) {
        return NULL;
    }
     ...
     *
     *     conf->address = NULL;
     *     conf->hmac.key = {0, NULL};
     *     conf->hmac.ipad = {0, NULL};
     *     conf->hmac.opad = {0, NULL};
     */
...
static char *ngx_http_stunnel_merge_loc_conf(ngx_conf_t *cf, void *parent, void *child) {
    ngx_http_stunnel_loc_conf_t *prev = parent;
    ngx_http_stunnel_loc_conf_t *conf = child;
...
static void ngx_http_stunnel_variable_set_time(ngx_http_request_t *r,
    ...
    if (ms == (ngx_msec_t)NGX_ERROR) {
        ngx_log_error(NGX_LOG_ERR, r->connection->log, 0,
                      "stunnel: invalid msec \"%V\" (ctx offset=%ui)", &s, data);
        return;
    }
...
static ngx_int_t ngx_http_stunnel_add_variables(ngx_conf_t *cf) {
    ...
    for (v = ngx_http_stunnel_vars; v->name.len; v++) {
